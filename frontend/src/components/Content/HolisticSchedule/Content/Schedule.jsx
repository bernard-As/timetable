...
const Schedule = observer(({ id, model }) => {
	...
	useEffect(() => {
		...
		timeInterval,
		selectedScheduleType,
		rootStore.holisticScheduleStore.schedule_created,
	]);
	...
	useEffect(() => {
		const compareTimeSlots = (timeSlot, courseStart, courseEnd) => {
			...
			if (
				slotStartTime.isSame(dataStartTime) &&
				slotEndTime.isSame(dataEndTime)
			) {
				return true;
			}
		...
		const convertDateToDayOfWeekId = (date) => {
			...
			return rootStore.holosticScheduleContentStore.daysIndex.find(
				(d) => d.name === dayOfWeek
			)?.id;
		};
		const getDayData = (timeSlot, day, data) => {
			let isDay = false;
			if (data.day !== (null || undefined) && day === data.day) isDay = true;
			else if (
				data.date !== (null || undefined) &&
				day === convertDateToDayOfWeekId(data.date)
			)
				isDay = true;
			if (!isDay) return false;
			else if (compareTimeSlots(timeSlot, data.start, data.end)) return true;
			return false;
		};
		const convertToData = () => {
			let newSh = [];
	...
	return (
		<>
			...
			{(selectedScheduleType !== 5 || selectedScheduleType!==6||selectedScheduleType!==7)? (
				timeSlots.length > 0 && (
					<Table
						columns={columns}
