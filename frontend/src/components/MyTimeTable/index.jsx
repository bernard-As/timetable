...
import {
	...
	getDayString,
	ScheduleCell,
} from "../Content/HolisticSchedule/Content/AdditionalRendering";
...
const MyTimetable = () => {
	...
	const [selectedScheduleType, setselectedScheduleType] = useState(5); //default 'All'
	...
	useEffect(() => {
		if (selectedScheduleType === 3) {
			...
			setColumns1([defaultTimeslotCol, ...week1Columns]);
			setColumns2([defaultTimeslotCol, ...week2Columns]);
			setColumns3([defaultTimeslotCol, ...week3Columns]);
		} else if (selectedScheduleType === 6 || selectedScheduleType === 7) {
			const week1Columns = [];
			for (
				let i = resit_makeup_days.week1.start;
				i <= resit_makeup_days.week1.end;
				i++
			) {
				const m = `${i}-06-2025`;
				week1Columns.push({
			...
			setColumns([
				...
				...rootStore.holosticScheduleContentStore.daysIndex.map(
					({ id, name, ...rest }) => ({
						title: name,
						dataIndex: id,
						key: id,
	...
	useEffect(() => {
		...
		const getMyData = async () => {
			await PrivateDefaultApi.get("my_schedule/")
				.then((res) => {
					if (selectedScheduleType !== "" && selectedScheduleType !== "All") {
						const filteredData = res.data.filter(
							(item) => item.type === selectedScheduleType
						);
	...
	useEffect(() => {
		...
		rootStore.holosticScheduleContentStore.daysIndex.map((d) => {
			...
			timeSlots.map((t) => {
				sc[`${t.start} - ${t.end}`] = data?.filter((dt) =>
					getDayData(t, d.id, dt)
				);
			});
	...
	useEffect(() => {
		...
		timeSlots.map((t) => {
			...
			if (t !== undefined)
				mobCol = [
					...mobCol.filter((m) => m.title !== timespanStrig),
					{
						title: (
							<span id={`${t.start} - ${t.end}`}>
						...
						key: timespanStrig,
						render: (_, record) => (
							<ScheduleCell record={record[timespanStrig]} />
						),
					},
				];
		});
	...
	const handleChageDisplay = (selected) => {
		if (selected === "mobile") setisDesktopViewType(false);
		else setisDesktopViewType(true);
	};
	...
	return (
		<>
			...
			{[6, 7, 5].includes(selectedScheduleType) && (
				<FloatButton.Group
					trigger="click"
					type="primary"
					style={{
					...
					{columns2.length > 0 && selectedScheduleType === 5 && (
						<FloatButton
					...
					{columns3.length > 0 && selectedScheduleType === 5 && (
						<FloatButton
