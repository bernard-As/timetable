...
import { DepartmentInt, FacultyInt, LecturerInt, ProgramInt, StudentInt } from "../../interfaces"
...
const Create:React.FC = () => {
    ...
    const [formData, setFormData] = useState<StudentInt>({
        id: 0,
        first_name:"",
        last_name:'',
        email:'',
    ...
    useEffect(()=>{
        document.title = 'Student Setting'
    })
    useEffect(()=>{
        setTimeout(() => {
            setRequestStatus(0)
    ...
    const handleSubmit =async  (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            formData.username = formData.first_name.replace(' ','')+formData.studentId
            console.log(formData)
            const response = await PrivateDefaultApi.post('student/',formData)
    ...
    return(
        ...
        {
            requestStatus!==0 && (
                requestStatus === 2?<Alert title="Student Id Already exist"  icon={'info'}/>:
            ...
            <form className="row g-3" onSubmit={handleSubmit} autoComplete="off">
                ...
                <label htmlFor="validationCustom01" className="form-label">First Name</label>
                  ...
                  <input 
                    ...
                    className="form-control"  
                    name='email'  
                    placeholder="Eg: robert@prof.com" 
...
const Edit:React.FC<{id:number}> =(id) =>{
    ...
    useEffect(()=>{
        PrivateDefaultApi.get(`/student/${id.id}/`)
        .then((res:any)=>{
        setShowEdit(true)
    ...
    const handleSubmit =async  (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const response = await PrivateDefaultApi.put(`student/${id.id}/`,formData)
            setRequestStatus(response.status)
            setShowEdit(false)
        } catch (error:any) {
            try{
    ...
    return (
        ...
        </div>)}
        {
            ...
            :requestStatus===409?
                (<Alert title={'Student Already exist'} icon={'error'}/>)
            :requestStatus!==0?
                (<RequestHandler status={requestStatus}/>)
            :''
    ...
    function handleDelete(id: number){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    PrivateDefaultApi.delete(`/student/${id}/`)
                    .then(()=>{
