...
const Create:React.FC = () => {
    ...
    useEffect(()=>{
        document.title = 'Department Setting'
    })
    useEffect(()=>{
        setTimeout(() => {
            setRequestStatus(0)
    ...
    const handleSubmit =async  (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const response = await PrivateDefaultApi.post('department/',formData,{
                headers: {
            ...
            <form className="row g-3" onSubmit={handleSubmit} autoComplete="off">
                <div className="col-md-6">
                  ...
                  <input 
                    ...
                    required
                  />
                  <label htmlFor="building">Select the Faculty:</label>
                    <select name="faculty" id="faculty" onChange={handleChange} className="form-select form-select-lg mb-2" >
                      <option value="">Select a faculy</option>
                      {
                          faculties?.map((b: any)=>(
...
const Edit:React.FC<{id:number}> =(id) =>{
    ...
    useEffect(()=>{
        PrivateDefaultApi.get(`/department/${id.id}/`)
        .then((res:any)=>{
            setShowEdit(true)
...
const List:React.FC = () => {
    ...
    useEffect(()=>{
        ...
        PrivateDefaultApi.get('department/')
        .then((res:any)=>{
        ...
        return (
            data.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>{faculties?.find((f)=>f.id===item.faculty)?.shortname}</td>
    ...
    function handleDelete(id: number){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    PrivateDefaultApi.delete(`/department/${id}/`)
                    .then(()=>{
