...
import { DepartmentInt,FacultyInt,ProgramInt } from "../../interfaces"
const Create:React.FC = () => {
    const [requestStatus, setRequestStatus] = useState(0)
    const [formData, setFormData] = useState<ProgramInt>({
        id:0,
        name: '',
        shortname: '',
        color: '',
    ...
    useEffect(()=>{
        document.title = 'Programs Setting'
    })
    useEffect(()=>{
        setTimeout(() => {
            setRequestStatus(0)
            ...
            <form className="row g-3" onSubmit={handleSubmit} autoComplete="off">
                <div className="col-md-6">
                  ...
                  <label htmlFor="department">Select the Department:</label>
                    <select name="department" id="department" onChange={handleChange} className="form-select form-select-lg mb-2"  required>
        ...
        return (
            data.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>{deps?.find((d)=>d.id===item.department)?.shortname}</td>
