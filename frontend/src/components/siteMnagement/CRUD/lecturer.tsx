...
import { DepartmentInt, FacultyInt, LecturerInt, ProgramInt } from "../../interfaces"
...
const Create:React.FC = () => {
    ...
    useEffect(()=>{
        document.title = 'Lecturer Setting'
    })
    useEffect(()=>{
        setTimeout(() => {
            setRequestStatus(0)
    ...
    const handleSubmit =async  (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            formData.username = formData.first_name.replace(' ','')+formData.lecturerid
            console.log(formData)
            const response = await PrivateDefaultApi.post('lecturer/',formData)
...
const Edit:React.FC<{id:number}> =(id) =>{
    ...
    useEffect(()=>{
        PrivateDefaultApi.get(`/lecturer/${id.id}/`)
        .then((res:any)=>{
        setShowEdit(true)
    ...
    const handleSubmit =async  (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const response = await PrivateDefaultApi.put(`lecturer/${id.id}/`,formData)
            setRequestStatus(response.status)
            setShowEdit(false)
        } catch (error:any) {
            try{
...
const List:React.FC = () => {
    ...
    useEffect(()=>{
        ...
        PrivateDefaultApi.get('/lecturer/')
        .then((res:any)=>{
    ...
    function handleDelete(id: number){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    PrivateDefaultApi.delete(`/lecturer/${id}/`)
                    .then(()=>{
